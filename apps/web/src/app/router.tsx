...
import { QueryRouteLoading } from "@/features/explore/QueryLoadingState";
...
const SchemasPage = React.lazy(() =>
  import("@/features/schemas/SchemasPage").then((m) => ({ default: m.SchemasPage })),
);
...
/** Redirect the legacy /query path to /explore, preserving the query string. */
function QueryRedirect() {
  return <Navigate to={{ pathname: "/explore", search: window.location.search }} replace />;
}
...
export const router: ReturnType<typeof createBrowserRouter> = createBrowserRouter([
  {
    ...
    children: [
      ...
      // Legacy path kept as a redirect (preserving query string) so existing
      // links and bookmarks continue to resolve to Explore.
      { path: "query", element: <QueryRedirect /> },
      {
        path: "schemas",
        element: (
          <Lazy fallback={<SchemasRouteFallback />}>
            <SchemasPage />
          </Lazy>
        ),
      },
